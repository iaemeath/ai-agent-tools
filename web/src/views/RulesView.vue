...
import type { RuleInfo } from '../types/tool';

const { t } = useI18n();
const { tool } = useTool();

const items = ref<RuleInfo[]>([]);
...
// supplies the resource key ('rules-order') and page-specific grouping data.
const drag = useDragOrder('rules-order');
...
async function reload() {
	...
	raw.value = '';
	try {
		items.value = await api.listRules(tool.value);
		// Auto-select the first global rule for immediate content.
		const g = globalItems.value.find((i) => i.scope === 'global');
		if (g) await selectRule(g);
	} catch (e) {
...
async function selectRule(r: RuleInfo) {
	...
	try {
		const res = await api.readRule(r.path, tool.value);
		raw.value = res.raw;
	} catch (e) {
		raw.value = '```\n' + (e as Error).message + '\n```';
	} finally {
...
const projectGroups = computed<[string, RuleInfo[]][]>(() => {
...
<template>
  <div class="rules-view">
    ...
    <div v-else-if="items.length === 0" class="state empty-state">{{ t('rule.empty') }}</div>

    <div v-else class="split-layout">
      <!-- Left: grouped file list (draggable cards) -->
      <div class="pane pane-left" :style="{ width: leftWidth + 'px', flexShrink: 0 }">
        <div class="pane-header">
          <span class="pane-title">{{ t('nav.rules') }}</span>
          <span class="pane-meta">{{ items.length }}</span>
        </div>
        <div class="pane-body list-body">
          <!-- Global group -->
          <div class="group-label">🌐 {{ t('rule.groupGlobal') }}</div>
          <div v-if="globalItems.length === 0" class="state small">{{ t('rule.empty') }}</div>
          <div v-else class="rule-grid">
            ...
            >
              ...
              <div v-if="r.description" class="rule-desc" :title="r.description">{{ r.description }}</div>
              <div class="rule-meta">{{ r.lineCount }} {{ t('rule.lines') }}</div>
            </div>
          </div>
      ...
      <div class="pane pane-right">
        ...
        <div v-else class="state center">{{ t('rule.noSelection') }}</div>
      </div>
    </div>
  </div>
</template>
...
/* ---- Rule cards (draggable grid items) ---- */
.group-label {
  font-size: 11px;
  font-weight: 600;
  color: var(--el-text-color-secondary);
