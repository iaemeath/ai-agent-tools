...
class I18nState {
	...
	get availableLocales(): LocaleInfo[] { return localeInfos; }
	get currentLabel(): string { return localeInfos.find((l) => l.code === this.locale)?.label ?? 'EN'; }
	get nextLocale(): Locale { const idx = localeInfos.findIndex((l) => l.code === this.locale); return localeInfos[(idx + 1) % localeInfos.length].code; }
}
